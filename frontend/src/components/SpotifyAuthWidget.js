export function renderSpotifyLoggedIn(container, profile, onLogout) {
  ...
  document.getElementById('spotify-logout')?.addEventListener('click', onLogout);
}

export function renderSpotifyLoggedOut(container, onLogin) {
  container.innerHTML = `
  ...
  document.getElementById('spotify-login-btn')?.addEventListener('click', onLogin);
}

export default { renderSpotifyLoggedIn, renderSpotifyLoggedOut };
